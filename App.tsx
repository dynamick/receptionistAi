...
import { Mic, MicOff, MessageSquare, Terminal, RefreshCcw, Power, Volume2, UserCircle2 } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Character State
  const [currentAvatar, setCurrentAvatar] = useState<'kai' | 'mick'>('kai');
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            if (message.toolCall) {
              for (const fc of message.toolCall.functionCalls) {
                ...
                let responseText = "Azione eseguita.";
                
                switch (fc.name) {
                  case 'switchAvatar':
                    const target = fc.args.character as 'kai' | 'mick';
                    setCurrentAvatar(target);
                    responseText = `Avatar cambiato in ${target}.`;
                    break;
        ...
        config: {
          ...
          systemInstruction: `Sei un'assistente 3D amichevole. Puoi cambiare identità tra Kai (femmina) e Mick (maschio).
Comunichi via audio e usi il linguaggio del corpo.
Se l'utente vuole cambiare personaggio, chiama Mick o chiede di cambiare avatar, usa 'switchAvatar' specificando il nome.
...
Presentati in base all'avatar attivo: attualmente sei ${currentAvatar}.`,
          speechConfig: {
          ...
          tools: [{
            functionDeclarations: [
              { 
                name: 'switchAvatar', 
                description: 'Cambia il personaggio attivo.', 
                parameters: { 
                  type: Type.OBJECT, 
                  properties: { 
                    character: { type: Type.STRING, enum: ['kai', 'mick'], description: 'Il nome del personaggio da attivare.' } 
                  },
                  required: ['character']
                } 
              },
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-full bg-slate-900 text-slate-100 overflow-hidden font-sans">
      <div className="relative flex-1 h-2/3 md:h-full">
        <Scene 
          modelUrl={AVATARS[currentAvatar]}
        ...
        <div className="absolute top-6 left-6 flex flex-col gap-2">
          <div className="flex items-center gap-4 bg-slate-900/60 backdrop-blur-md px-4 py-2 rounded-full border border-slate-700">
          ...
          <div className="flex items-center gap-2 bg-indigo-900/40 backdrop-blur-md px-4 py-2 rounded-full border border-indigo-700/50">
            <UserCircle2 size={16} className="text-indigo-400" />
            <span className="text-xs font-semibold uppercase">Avatar: {currentAvatar}</span>
          </div>
        </div>

        {(lastTranscript || userTranscript) && (
          <div className="absolute top-24 left-1/2 -translate-x-1/2 w-full max-w-lg px-4 pointer-events-none">
            <div className="bg-black/40 backdrop-blur-sm p-4 rounded-xl border border-white/10 text-center">
              ...
              {lastTranscript && <p className="text-white text-lg font-medium capitalize">{currentAvatar}: {lastTranscript}</p>}
            </div>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-500 text-center px-4">
              ...
              <p>Connettiti e interagisci. Puoi chiederle di ballare, saltare o persino cambiare avatar chiamando "Mick"!</p>
            </div>
          ) : (
            messages.map((m, i) => (
              <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                ...
                <span className="text-[10px] text-slate-500 mt-1 uppercase">
                  {m.role === 'user' ? 'Tu' : currentAvatar} • {new Date(m.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </span>
              </div>
            ))
          )}
